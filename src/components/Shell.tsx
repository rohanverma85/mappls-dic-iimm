...
import { Bell, Building2, ChevronDown, CircleHelp, ClipboardCheck, Construction, CreditCard, FileBarChart, FolderKanban, Gauge, Landmark, LogOut, Menu, PackageSearch, PanelLeftClose, Search, Settings2, ShieldCheck, Smartphone, UserCog, Users, Wrench, X } from 'lucide-react';
import type { Role, Session } from '../../shared/types';
import { ROLE_LABELS } from '../../shared/types';
import { api } from '../api';
import { Logo } from './Logo';

export type AppPage = 'dashboard'|'tenants'|'onboarding'|'users'|'projects'|'assets'|'attendance'|'inspections'|'defects'|'payments'|'citizen'|'helpdesk'|'reports'|'notifications'|'activity'|'search';

const navByRole: Record<Role, { group:string; items:{page:AppPage; label:string; icon:typeof Gauge}[] }[]> = {
  tenant_admin:[
    {group:'PLATFORM',items:[{page:'dashboard',label:'Cross-tenant overview',icon:Gauge},{page:'tenants',label:'Tenant management',icon:Building2},{page:'onboarding',label:'Provision new tenant',icon:Settings2},{page:'activity',label:'Access & activity log',icon:ShieldCheck},{page:'reports',label:'Platform reports',icon:FileBarChart}]},
  ...
  authority:[
    {group:'OPERATIONS',items:[{page:'dashboard',label:'Operations dashboard',icon:Gauge},{page:'projects',label:'Projects',icon:FolderKanban},{page:'assets',label:'Assets',icon:PackageSearch},{page:'inspections',label:'Inspections',icon:ClipboardCheck},{page:'defects',label:'Defects',icon:Construction},{page:'payments',label:'Payment approvals',icon:CreditCard}]},
  ...
  maker:[
    ...
    {group:'TOOLS',items:[{page:'assets',label:'Assigned assets',icon:PackageSearch},{page:'notifications',label:'Notifications',icon:Bell},{page:'helpdesk',label:'Helpdesk',icon:CircleHelp},{page:'search',label:'Search',icon:Search}]},
  ],
  checker:[
    ...
    {group:'TOOLS',items:[{page:'projects',label:'Assigned projects',icon:FolderKanban},{page:'reports',label:'Agency reports',icon:FileBarChart},{page:'notifications',label:'Notifications',icon:Bell},{page:'helpdesk',label:'Helpdesk',icon:CircleHelp},{page:'search',label:'Search',icon:Search}]},
  ],
  citizen:[
